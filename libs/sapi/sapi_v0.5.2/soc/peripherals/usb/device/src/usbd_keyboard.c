...
static ErrorCode_t Keyboard_GetReport(USBD_HANDLE_T hHid, USB_SETUP_PACKET *pSetup,
...
static ErrorCode_t Keyboard_SetReport(USBD_HANDLE_T hHid, USB_SETUP_PACKET *pSetup,
...
ErrorCode_t usbDeviceKeyboardInit(USBD_HANDLE_T hUsb, USB_INTERFACE_DESCRIPTOR *pIntfDesc,
...
void usbDeviceKeyboardTasks(void) {
    ...
    if (USB_IsConfigured(g_keyBoard.hUsb)) {
        ...
        if (g_keyBoard.tx_busy == 0) {
            ...
            Keyboard_UpdateReport();
            USBD_API->hw->WriteEP(g_keyBoard.hUsb, HID_EP_IN, &g_keyBoard.report[0],
...
bool_t usbDeviceKeyboardReceiveFromHostCallbackSet(callBackFuncPtr_t keyboardReceiveCallback) {
    bool_t retVal = TRUE;
...
bool_t usbDeviceKeyboardCheckKeysCallbackSet(callBackFuncPtr_t checkForPressedKeysCallback) {
    bool_t retVal = TRUE;
