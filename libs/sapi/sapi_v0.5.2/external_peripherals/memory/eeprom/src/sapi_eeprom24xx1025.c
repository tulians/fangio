...
bool_t eeprom24xx1025Init(Eeprom24xx1025_t* eeprom, int32_t i2c, bool_t A0, bool_t A1,
                          int32_t gpioA0, int32_t gpioA1, int32_t gpioPower, int32_t gpioWP,
...
// Byte Write
bool_t eeprom24xx1025WriteByte(Eeprom24xx1025_t* eeprom, uint32_t memoryAddress,
                               uint8_t byteToWrite) {
    ...
    retVal = i2cWrite(eeprom->i2c, eeprom24xx1025I2cAddress(eeprom, memoryAddress), dataToWrite, 3,
                      TRUE);
...
// Page Write
bool_t eeprom24xx1025WritePage(Eeprom24xx1025_t* eeprom, uint32_t page, uint8_t* byteBuffer,
    ...
    // uint8_t* transmitDataBuffer, uint16_t transmitDataBufferSize,
    // bool_t sendWriteStop
    retVal = i2cWrite(eeprom->i2c, eeprom24xx1025I2cAddress(eeprom, memoryAddress), dataToWrite,
                      (byteBufferSize + 2), TRUE);
...
bool_t eeprom24xx1025ReadCurrentAddress(Eeprom24xx1025_t* eeprom, uint32_t memoryAddress,
                                        uint8_t* readedByte) {
    bool_t retVal = TRUE;  // True if OK
    ...
    // bool_t sendReadStop
    retVal = i2cRead(eeprom->i2c, eeprom24xx1025I2cAddress(eeprom, memoryAddress), (uint8_t*)0, 0,
                     FALSE, readedByte, 1, TRUE);

    return retVal;  // read correct
}

// Random Read
bool_t eeprom24xx1025ReadRandom(Eeprom24xx1025_t* eeprom, uint32_t memoryAddress,
                                uint8_t* readedByte) {
    bool_t retVal = TRUE;  // True if OK
    ...
    // uint8_t* reciveDataBuffer, uint16_t reciveDataBufferSize,
    // bool_t sendReadStop
    retVal = i2cRead(eeprom->i2c, eeprom24xx1025I2cAddress(eeprom, memoryAddress), addressToRead, 2,
                     FALSE, readedByte, 1, TRUE);
...
// Sequential Read
bool_t eeprom24xx1025ReadSequential(Eeprom24xx1025_t* eeprom, uint32_t address, uint8_t* byteBuffer,
                                    uint32_t byteBufferSize) {
    bool_t retVal = TRUE;  // True if OK
    ...
    // uint8_t* reciveDataBuffer, uint16_t reciveDataBufferSize,
    // bool_t sendReadStop
    retVal = i2cRead(eeprom->i2c, eeprom24xx1025I2cAddress(eeprom, address), addressToRead, 2,
                     FALSE, byteBuffer, byteBufferSize, TRUE);
...
bool_t eeprom24xx1025TestWriteBytes(uint32_t startingPage, uint8_t* buff, uint32_t buffSize) {
    ...
    eeprom24xx1025Init(&eeprom24A1025I, I2C0, 0, 0, -1, -1, -1, -1, EEPROM24xx1025_PAGE_SIZE,
                       EEPROM_1024_K_BIT);
 ...
 * Performs a test on the 24xx1025 EEPROM memory.
 */
bool_t eeprom24xx1025TestReadSequential(uint32_t address, uint8_t* byteBuffer,
                                        uint32_t byteBufferSize) {
    boardInit();
    ...
    if (FALSE ==
        eeprom24xx1025ReadSequential(&eeprom24A1025I, address, byteBuffer, byteBufferSize)) {
 ...
 */
bool_t eeprom24xx1025TestReadCurrentAddress(uint32_t dummyReadAddress, uint8_t* byteBuffer) {
    uint8_t readByte = 0;
    uint32_t i = 0;
    ...
    for (i = 0; i < 10; i++) {
        if (eeprom24xx1025ReadCurrentAddress(&eeprom24A1025I, dummyReadAddress + i, &readByte)) {
            debugPrintChar(readByte);
        } else {
