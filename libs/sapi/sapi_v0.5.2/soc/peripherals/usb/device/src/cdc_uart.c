...
static ErrorCode_t UCOM_bulk_hdlr(USBD_HANDLE_T hUsb, void *data, uint32_t event) {
    ...
    switch (event) {
        ...
        case USB_EVT_IN:
            ...
            if (count) {
                pUcom->usbTxBusy = 1;
                count = USBD_API->hw->WriteEP(pUcom->hUsb, USB_CDC_IN_EP,
                                              &pUcom->rxBuf[g_uCOM.rxBuf_usbIndex], count);
        ...
        case USB_EVT_OUT:
            if ((Chip_UART_GetIntsEnabled(LPC_UART) & UART_IER_THREINT) == 0) {
                ...
                pUcom->txBuf_uartIndex = Chip_UART_Send(
                    LPC_UART, &pUcom->txBuf[g_uCOM.txBuf_uartIndex], pUcom->txBuf_count);
...
static ErrorCode_t CDC_ep0_override_hdlr(USBD_HANDLE_T hUsb, void *data, uint32_t event) {
    ...
    if ((event == USB_EVT_OUT) && (pCtrl->SetupPacket.bmRequestType.BM.Type == REQUEST_CLASS) &&
        ...
        /* Check which CDC control structure to use. If epin_num doesn't have BIT7 set then we are
           at wrong offset so use the old CDC control structure. BIT7 is set for all EP_IN
           endpoints.
...
ErrorCode_t cdcUartLpcInit(USBD_HANDLE_T hUsb, USB_CORE_DESCS_T *pDesc,
    ...
    cdc_param.cif_intf_desc =
        (uint8_t *)find_IntfDesc(pDesc->high_speed_desc, CDC_COMMUNICATION_INTERFACE_CLASS);
    cdc_param.dif_intf_desc =
        (uint8_t *)find_IntfDesc(pDesc->high_speed_desc, CDC_DATA_INTERFACE_CLASS);
...
void cdcUartCallbackSet(uartEvents_t event, callBackFuncPtr_t callbackFunc, void *callbackParam) {}
